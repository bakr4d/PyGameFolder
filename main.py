...
# Colors
WHITE = (255, 255, 255)
GRAY = (150, 150, 150)  # رمادي باهت للتعادل
RED = (255, 0, 0)
GREEN = (0, 255, 0)
BLACK = (0, 0, 0)
...
def draw_winning_line(player):
    ...
    for row in range(BOARD_ROWS):
        if all(board[row][col] == player for col in range(BOARD_COLS)):
            pygame.draw.line(screen, GREEN if player == 1 else RED,
...
while running:
    ...
    if not game_over:
        ...
        draw_board()
        draw_figures(GREEN if winner == 1 else RED if winner == 2 else GRAY)
        draw_lines(GREEN if winner == 1 else RED if winner == 2 else GRAY)
        if winner != 0:
